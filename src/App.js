  ...
  return (
    ...
    <Link to='/' className="logo">
      ...
      <header className="App-header">
        <form
          onSubmit={async (e) => {
            ...
            try {
              ...
              await getWeatherData(e.target.input.value)
        ...
        {data ? (
          <div className="data">
            <p>Current Temperature is: <span className="highlight">{temp} {temp && '°C'} ({condition}) </span></p>
            <p>Current Time is: <span className="highlight">{time? time: '...'}</span></p>
          </div>
        ) : content}
      </header>
    </div>
